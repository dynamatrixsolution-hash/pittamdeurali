...
const connectDB = async () => {
  try {
    let mongoUrl = process.env.MONGODB_URL || process.env.MONGO_URI;
    if (!mongoUrl) {
      console.warn('MONGODB_URL or MONGO_URI env variable not found. Falling back to local MongoDB (mongodb://127.0.0.1:27017/pittamdeurali).');
